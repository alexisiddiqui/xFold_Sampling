...
class BoltzParams:
    ...
    max_paired_seqs: List[int] = field(default_factory=lambda: [8192])
    ...
    num_seeds: int = 1  # Number of seeds to run for each parameter combination
    msa_dropout: list[float] = field(default_factory=lambda: [0.0, 0.15, 0.25, 0.5])
    msa_z_dropout: list[float] = field(default_factory=lambda: [0.0, 0.15, 0.25, 0.5])
    pairformer_dropout: list[float] = field(default_factory=lambda: [0.0, 0.15, 0.25, 0.5])
...
def main():
    ...
    for (
        ...
        params.msa_dropout,
        params.msa_z_dropout,
        params.pairformer_dropout,
    ):
        suffix = f"steps{sampling_steps}_recycle{recycling_steps}_diff{diffusion_samples}_scale{step_scale}_maxseqs{max_paired_seqs}_msa_drop{msa_dropout}_msa_z_drop{msa_z_dropout}_pf_drop{pairformer_dropout}"
