...
def combine_af_json_info(af_dir: str):
    ...
    dir_names = [dir_name for dir_name in dir_names if os.path.isdir(os.path.join(af_dir, dir_name))]
    dir_names.sort(key=lambda x: int(x.split("_")[1]), reverse=True)  # Sort by max MSA size in descending order
    ...
    with ThreadPoolExecutor() as executor:  # Adjust number of workers as needed
        future_to_dir = {executor.submit(process_directory, os.path.join(af_dir, dir_name)): dir_name for dir_name in dir_names}
